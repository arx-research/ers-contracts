...
export default class MerkleTree {
  ...
  constructor(elements: Buffer[]) {
    ...
    this.bufferElementPositionIndex = this.elements.reduce<{ [hexElement: string]: number }>((memo, el, index) => {
      memo[ethereumjsUtil.bufferToHex(el)] = index
      return memo
    }, {})
  ...
  static combinedHash(first: Buffer, second: Buffer): Buffer {
    ...
    return ethereumjsUtil.keccak256(MerkleTree.sortAndConcat(first, second))
  }
  ...
  getProof(el: Buffer) {
    let idx = this.bufferElementPositionIndex[ethereumjsUtil.bufferToHex(el)]
