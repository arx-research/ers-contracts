...
export class Blockchain {
  ...
  public async getCurrentTimestamp(): Promise<BigNumber> {
    return BigNumber.from((await this._provider.getBlock(await this._provider.getBlockNumber())).timestamp);
  }
  ...
  public async getLatestBlockNumber(): Promise<BigNumber> {
    return BigNumber.from((await this._provider.getBlock("latest")).number);
  }
