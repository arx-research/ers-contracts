...
import { IProjectRegistrarFactory } from "./interfaces/IProjectRegistrarFactory.sol";

/**
 * @title ProjectFactory
 * @author Arx
 *
 * @notice Contract responsible for tracking and permissioning Project Registrar Factories. Governance has the ability to add and remove
 * factories as needed.
 */
contract ProjectRegistry is Ownable {
    ...
    mapping(IProjectRegistrarFactory=>bool) public registrarFactories;  // Mapping indicating if address is a registered DeveloperRegistrarFactory
     ...
     * @param _factories                 Array of ProjectRegistrarFactory contracts
     */
    function initialize(
        IERS _ers,
        IProjectRegistrarFactory[] calldata _factories
    )
        external
        onlyOwner
    {
     ...
     */
    function createNewProjectRegistrar(IProjectRegistrarFactory _factory) external returns(address) {
        require(registrarFactories[_factory], "Factory must be approved ProjectRegistrarFactory");
        ...
        address newRegistrar = IProjectRegistrarFactory(_factory).deployProjectRegistrar(msg.sender);
    ...
    /**
     * @notice ONLY OWNER: Add a new ProjectRegistrarFactory that can be used for creating new DeveloperRegistrars. 
     *
     * @param _factory             Address of ProjectRegistrarFactory to add
     */
    function addRegistrarFactory(IProjectRegistrarFactory _factory) external onlyOwner {
        _addRegistrarFactory(_factory);
    }

    /**
     * @notice ONLY OWNER: Remove a ProjectRegistrarFactory so that it can't be used for creating new DeveloperRegistrars.
     *
     * @param _factory             Address of ProjectRegistrarFactory to add
     */
    function removeRegistrarFactory(IProjectRegistrarFactory _factory) external onlyOwner {
    ...
    function _addRegistrarFactory(IProjectRegistrarFactory _factory) internal {
